...
namespace B1
{
  ...
  G4VPhysicalVolume *DetectorConstruction::ConstructWrite()
  {
    ...
    auto solidEnv = new G4Box("Envelope",                                           // its name
                              0.8 * env_sizeXY, 0.8 * env_sizeXY, 0.8 * env_sizeZ); // its size
    ...
    // Shape 1
    //
    G4Material *shape1_mat = nist->FindOrBuildMaterial("G4_Be");
    ...
    //
    G4GDMLParser parser;
    parser.Write("output.gdml", physWorld); // this->GetWorldVolume());
    return physWorld;
  }
